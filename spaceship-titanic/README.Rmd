...
```{r de-groups, fig.align = 'center', warning = FALSE}
...
Let's check the Area under the Curve (AUC) for this model, which can be interpreted as 

### Validation

We'll now use this model to predict over our validation set, then we'll check performance metrics such as sensitivity and specificity. Note that the sensitivity is our True Positive Rate, and specificity is our True Negative Rate.
...
Our sensitivity and specificity are not too bad, although I'm sure we can get better with a more targeted model!
...
In validating this model, we see that it hasn't done much better than the random forest we set up earlier. Accuracy, sensitivity, and specificity are around the same as that of the random forest.
